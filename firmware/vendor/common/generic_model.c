 ...
 * @retval None
 */
//RD_EDIT: G_onoff_st_rsp_par_fill
 ...
 */
int mesh_tx_cmd_g_onoff_st(u8 idx, u16 ele_adr, u16 dst_adr, u8 *uuid,
	...
	return tx_rsp_flag;
#else
	return mesh_tx_cmd_rsp(op_rsp, (u8 *) &rsp, len, ele_adr, dst_adr, uuid,
			pub_md);
#endif
}
...
int mesh_g_onoff_st_rsp(mesh_cb_fun_par_t *cb_par) {
	...
	return mesh_tx_cmd_g_onoff_st(cb_par->model_idx, p_model->com.ele_adr,
			cb_par->adr_src, 0, 0, cb_par->op_rsp);
	//RD_EDIT: mesh_tx_cmd_g_onoff_st
}
 ...
 *   (0: success; others: error)
 */
int mesh_g_onoff_st_publish(u8 idx) {	// RD_EDIT:  g_onoff_st_publish
	...
	return mesh_tx_cmd_g_onoff_st(idx, ele_adr, pub_adr, uuid, p_com_md,
			G_ONOFF_STATUS);
}
 ...
 *   (0: success; others: error)
 */
int g_onoff_set(mesh_cmd_g_onoff_set_t *p_set, int par_len, int force_last,
		int idx, bool4 retransaction, st_pub_list_t *pub_list) {
	...
	if (p_set->onoff < G_ONOFF_RSV) {
		...
		memcpy(
				&level_set_tmp.tid,
				&p_set->tid,
				sizeof(mesh_cmd_g_level_set_t)
						- OFFSETOF(mesh_cmd_g_level_set_t,tid));
		level_set_tmp.level = get_light_g_level_by_onoff(idx, p_set->onoff,
				st_trans_type, force_last);
		int len_tmp = GET_LEVEL_PAR_LEN(
		...
		if (!err) {
			RD_mod_io_onoff(p_set->onoff,idx,0);
...
{
	static uint32_t count_onoff = 0;
	char buff[64] = { 0 };
	count_onoff++;
	sprintf(buff, "on/off: %d \n", count_onoff);
	uart_CSend(buff);
	int err = 0;
#if DEBUG_PUBLISH_REDUCE_COLLISION_TEST_EN
	rx_ack_num++;
	...
	{
		if(cb_par->adr_dst == ele_adr_primary) 			RD_Set_UpdateStt(1);
		else if(cb_par->adr_dst == ele_adr_primary+1) 	RD_Set_UpdateStt(2);
		else if(cb_par->adr_dst == ele_adr_primary+2) 	RD_Set_UpdateStt(3);
		else if(cb_par->adr_dst == ele_adr_primary+3) 	RD_Set_UpdateStt(4);
		else if(cb_par->adr_dst == ele_adr_primary+4) 	RD_Set_UpdateStt(5);
		else 										  	RD_Set_UpdateStt(0xff);
		//RD_Update_Relay_Stt(1);
	}

	st_pub_list_t pub_list = { { 0 } };
	err = g_onoff_set(p_set, par_len, 0, cb_par->model_idx,
			cb_par->retransaction, &pub_list);
	if (err) {
		return err;
	}
 ...
 *   (0: success; others: error)
 */
int mesh_tx_cmd_g_level_st(u8 idx, u16 ele_adr, u16 dst_adr, u8 *uuid,
...
#endif
	return mesh_tx_cmd_rsp(G_LEVEL_STATUS, (u8 *) &rsp, len, ele_adr, dst_adr,
			uuid, pub_md);
}
...
int mesh_g_level_st_rsp(mesh_cb_fun_par_t *cb_par) {
	model_g_light_s_t *p_model = (model_g_light_s_t *) cb_par->model;
	return mesh_tx_cmd_g_level_st(cb_par->model_idx, p_model->com.ele_adr,
			cb_par->adr_src, 0, 0);
}
...
int mesh_cmd_sig_g_level_set(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par) {
	...
	u8 *par_set = par;
	st_pub_list_t pub_list = { { 0 } };
	err = g_level_set_and_update_last(par_set, par_len, cb_par->op, light_idx,
			cb_par->retransaction, trans_type, 0, &pub_list);
	if (err) {
		return err;
	}
...
int is_cmd_with_tid(u8 *tid_pos_out, u16 op, u8 *par, u8 tid_pos_vendor_app) {
	...
	if (IS_VENDOR_OP(op)) {
		...
		cmd_with_tid = is_mi_cmd_with_tid_vendor(tid_pos_out, op, tid_pos_vendor_app);
#else
		cmd_with_tid = is_cmd_with_tid_vendor(tid_pos_out, op, par,
				tid_pos_vendor_app);
#endif
	} else {
		switch (op) {
...
const mesh_model_resource_t MeshSigModelResource[] = { {SIG_MD_CFG_SERVER,GET_SINGLE_MODEL_AND_COUNT(model_sig_cfg_s, 0)},
#if MD_CFG_CLIENT_EN
 ...
 * @note        
 */
u8* mesh_find_ele_resource_in_model(u16 ele_adr, u32 model_id, bool4 sig_model,
		u8 *idx_out, int set_flag) {
	u8 *p_model = 0;
	*idx_out = 0;
			...
			if (p_source->model_id == model_id) {
				if (p_source->multi_flag) {
					foreach(i,p_source->model_cnt) {
						...
						model_common_t *p_com =
								(model_common_t *) ((u8 *) p_source->p_model
										+ p_source->size * i);
						if ((ele_adr == p_com->ele_adr) || (set_flag && (0
								== p_com->ele_adr))) {
							p_model = (u8 *) p_com;
							*idx_out = i;
							break;
