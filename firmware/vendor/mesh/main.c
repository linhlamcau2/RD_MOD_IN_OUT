...
{
	char buff[40];
	sprintf(buff,"tan duong: %d %d %d %d %d\n",reg_gpio_pa_in ,reg_gpio_pc_in,reg_irq_pri,reg_irq_src3,reg_irq_en);
	uart_CSend(buff);
//	RD_ev_log("irp: %d %d\n",reg_gpio_pa_in, reg_gpio_pc_in); //reg_irq_src, reg_irq_pri
//	irq_gpio_user_cnt++;
	return;
}
